...
export default function AdminDashboard() {
  ...
  React.useEffect(() => {
    ...
    const fetchStats = () => {
      setLoading(true);
      import('../../lib/api').then(({ serviceHubAPI }) => {
        serviceHubAPI.status('ADMIN').then(r => {
          if (!mounted) return;
          setStats(r.data);
          setError(null);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-6 max-w-[1440px] mx-auto">
        {/* Service Hub summary cards */}
        {loading && (
          <div className="mb-4 text-sm text-gray-500">Loading dashboard...</div>
        )}
        {error && (
          <div className="mb-4 text-sm text-red-600">Unable to load dashboard</div>
        )}
        <Outlet />
      </main>
    </div>
